...
function Start-UIASleep {
    ...
    if ($PSBoundParameters.ContainsKey('Seconds')) {
        # 秒が指定された場合
        [UIATools]::Sleep($Seconds * 1000)
    } elseif ($PSBoundParameters.ContainsKey('Milliseconds')) {
        # ミリ秒が指定された場合
        [UIATools]::Sleep($Milliseconds)
    } else {
...
function Get-UIAControls {
    ...
    # write-host "Found $($elements.Count) elements matched conditions"

    # ワイルドカードのName指定がある場合は、Nameプロパティをフィルタリングする
    if ($hasWildcardName) {
        # write-host "Apply filter by wildcard $($Name)"
...
function Invoke-UIAPane {
    <#
    .SYNOPSIS
        UIオートメーションのPane要素をClickします。
        InvokePatternやTogglePatternが動作しないことが多いので、ForceClickで直接クリックする。
    ...
    param(
        ...
        [Parameter(Mandatory, Position = 1)] $ParentWindow
    )
...
function Set-UIAComboBoxValue {
    ...
    # SetText出来ない場合は選択項目を列挙して選択する
    $selections = Get-UIAControls -Parent $Element -Type ([Windows.Automation.ControlType]::ListItem)
    ...
    foreach ($selection in $selections) {
        # write-host "Selection $($selection.Current.Name) is $($Value) ?"
        if ($selection.Current.Name -eq $Value) {
            $selectionPattern = $null
            if ($selection.TryGetCurrentPattern([Windows.Automation.SelectionItemPattern]::Pattern, [ref]$selectionPattern)) {
